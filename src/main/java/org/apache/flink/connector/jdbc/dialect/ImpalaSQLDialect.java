 ...
 */
public class ImpalaSQLDialect extends AbstractDialect {
	...
	public Optional<String> defaultDriverName() {
		return Optional.of("com.cloudera.impala.jdbc.Driver");
	}
	...
	public String getUpsertStatement(String tableName, String[] fieldNames, String[] uniqueKeyFields, boolean allReplace) {
	...
	private String buildConnectionConditions(String[] uniqueKeyFields) {
		return Arrays.stream(uniqueKeyFields).map(col -> "T1." + quoteIdentifier(col.trim()) + "=T2." + quoteIdentifier(col.trim())).collect(Collectors.joining(" and "));
	}
	...
	public String getSelectFromStatement(String tableName, String[] selectFields, String[] conditionFields) {
		...
		String fieldExpressions = conditionFields!=null? Arrays.stream(conditionFields)
			.map(f -> quoteIdentifier(f) + "=?")
			.collect(Collectors.joining(" AND ")):new String();
		return "SELECT " + selectExpressions + " FROM " +
			quoteIdentifier(tableName) + (fieldExpressions.length() > 0 ? " WHERE " + fieldExpressions : "");
	}
	/**
